      ...
      connectedServers.forEach(function(_server) {
        if(_server.isConnected()) {
          ...
          _server.command('system.$cmd', {ping: true}, function(err) {
            if(err) {
              if(Date.now() - now < customKeepAliveTimeout) {
                // only process error if it returned before the timeout
                // after the hanging mongos proxy is destroyed this query returns "sockets closed" error
                pingErrHandler(self, state, _server, err);
              }
            } else {
              if(state.logger.isDebug()) state.logger.debug(f('mongos ha process ping to %s successful', _server.name));
