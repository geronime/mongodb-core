...
//
// Remove a mongos from the list
State.prototype.removeMongos = function(server) {
  this.s.connectedServers = this.s.connectedServers.filter(function(s) {
    return !s.equals(server);
  });
...
State.prototype.pickServer = function(readPreference) {
  ...
  var connectedProxies = this.s.connectedServers.slice(0);
  ...
  for(var i = 0; i < connectedProxies.length; i++) {
    if(connectedProxies[i].s.isMasterLatencyMS < lowerBoundLatency) {
      lowerBoundLatency = connectedProxies[i].s.isMasterLatencyMS;
    }
  }

  // Filter out the possible servers
  connectedProxies = connectedProxies.filter(function(server) {
    if((server.s.isMasterLatencyMS <= (lowerBoundLatency + self.s.localThresholdMS))
      && server.isConnected()) {
      return true;
    }
  });
  ...
  if(connectedProxies.length == 0) throw new MongoError("no mongos proxy available");

  // Get proxy
  var proxy = connectedProxies[this.s.index % connectedProxies.length];
  // Update the index
  this.s.index = (this.s.index + 1) % connectedProxies.length;
  // Return the proxy
  return proxy;
}
...
var Mongos = function(seedlist, options) {
  ...
  // Default options for the customKeepAlive (if enabled)
  // options.customKeepAlive = true; // TESTING
  if(options.customKeepAlive) {
    options.customKeepAliveTimeout = options.customKeepAliveTimeout || 2000;
    options.customKeepAliveTries = options.customKeepAliveTries || 5;
  }
...
Mongos.prototype.connect = function(_options) {
  ...
  self.s.seedlist.forEach(function(e) {
    addMongosProxy(self, self.s.options, e.host, e.port);
  });
...
//
// refactored connectToProxy function
var connectToProxy = function(self, state, server) {
  setTimeout(function() {
...
//
// refactored function to add new mongos proxy
var addMongosProxy = function(self, options, host, port) {
  ...
  self.s.mongosState.disconnected(server);
  return server;
};
...
var mongosInquirer = function(self, state) {
  return function() {
    ...
    // Log the information
    if(state.logger.isDebug()) state.logger.debug(f('mongos ha proceess running, current state: %s', state.state));

    // Custom keep alive using ping cmd - only for connected servers
    if(state.options.customKeepAlive) {
      var connectedServers = state.mongosState.connectedServers();
      var customKeepAliveTimeout = state.options.customKeepAliveTimeout;
      connectedServers.forEach(function(_server) {
        if(_server.isConnected()) {
          var pingStartTs = Date.now();
          var pingTimeout = setTimeout(pingErrHandler, customKeepAliveTimeout, self, state, _server);
          _server.command('system.$cmd', {ping: true}, function(err) {
            // don't handle timed out pings here - they were already handled by the timeout handler
            var pingLatencyMS = Date.now() - pingStartTs;
            if(pingLatencyMS < customKeepAliveTimeout) {
              if(err) {
                pingErrHandler(self, state, _server, err);
              } else {
                if(state.logger.isDebug()) state.logger.debug(f('mongos ha process ping to %s successful in %d ms', _server.name, pingLatencyMS));
                // reset the counter of failed/timed out pings
                _server.s.customKeepAliveFailedPings = 0;
                // override the server's isMasterLatencyMS with the current pingLatencyMS
                // if this was a destroyed/re-added server that took some time before the connection
                // was really established and therefore isMaster actually yielded some result
                // it would never be selected by pickServer!
                // This is probably wrong place to do so; however it reflects the most recent latency.
                _server.s.isMasterLatencyMS = pingLatencyMS;
              }
    ...
    while(disconnectedServers.length > 0) {
      ...
      connectToProxy(self, state, server);
    }
...
//
// handler when a ping command on particular mongos proxy times out or fails
var pingErrHandler = function(self, state, server, err) {
  var serverState = server.s;
  serverState.customKeepAliveFailedPings = (serverState.customKeepAliveFailedPings || 0) + 1;
  if(state.logger.isDebug()) state.logger.debug(f('mongos ha process ping to %s failed: %s (%d/%d)', server.name, err || 'timeout', serverState.customKeepAliveFailedPings, state.options.customKeepAliveTries));
  if(serverState.customKeepAliveFailedPings >= state.options.customKeepAliveTries) {
    if(state.logger.isDebug()) state.logger.debug(f('mongos ha process: removing %s after %d failed/timed out pings', server.name, serverState.customKeepAliveFailedPings));
    // this server failed/timed out ping too many times, destroy and re-add
    state.mongosState.removeMongos(server);
    ...
    // add new mongos and actually destroy the old one
    var newServer = addMongosProxy(self, state.options, server.s.options.host, server.s.options.port);
    if(state.logger.isDebug()) state.logger.debug(f('mongos ha process: re-adding %s', newServer.name));
    try {
      server.destroy(false, false);
    } catch(err) {
      // do nothing, just kill the hanging server - improvement possible
    }
    // actual connection will be done on the next inquirer run
    // connectToProxy(self, state, newServer);
  }
}
